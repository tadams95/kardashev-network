...
- `LOW_TEMP_WARM_TAIL_MODE` — warm-tail (low-temp) signal-generation mode: `off` (default — no signals), `paper` (signals logged with mode='paper', naturally resolved with computed P&L, execute-tail-sells.ts skips them — used for shadow validation), or `live` (real Kalshi orders). **Warm-low is LIVE at $5/position since 2026-06-19** (the former `execute-tail-sells.ts` POSITION_SIZE hardcode blocker was fixed in commit `0939b49` — per-signal budget now used; the daily `kardashev-warmlow-stops` cron alerts on revert-to-paper triggers). Independent of `LOW_TEMP_SIGNAL_GENERATION_ENABLED` which gates the probability-model low-temp track separately. Toggle via PM2 reload: `echo 'LOW_TEMP_WARM_TAIL_MODE=paper' >> .env.local && pm2 reload kardashev-web --update-env`. **Paper budget caps:** paper signals respect `MAX_TOTAL_PAPER=60` (vs `MAX_TOTAL=8` for live) plus the same `MAX_PER_CITY=3` / `MAX_PER_CITY_TYPE=2` / `MAX_NE_CORRIDOR=5` sanity bounds. **Paper and live budgets are SEPARATE** — paper signals do NOT consume live's per-city/NE/total budget. `MAX_TOTAL_PAPER` raised from 30 to 60 on 2026-05-04 to accommodate 3 paper quadrants running simultaneously.
- `HOT_TAIL_HIGH_MODE` — hot-tail (high-temp) signal-generation mode: `off` (default), `paper` (signals tagged `direction='warm', temperatureType='high', mode='paper'`; execute-tail-sells.ts skips), or `live`. Hot-tail HIGH = sell YES on bracket ranges ≥6°F ABOVE forecast (the "heat wave" tail). Viability analysis 2026-05-03 returned GO (n=1265 events, mean EV +7.29¢, hit rate at +6°F = 3.00%, worst-day capped drawdown -$99 @ $20). **Per-city blacklist** (heat-wave-prone): AUS/BOS/LV/DAL/DC/PHIL/PHI/SEA. **Flip-to-live gate is STRICTER than the standard 30-trade rule:** requires 60-90 days minimum AND covers at least one summer heat-wave week AND 14-day rolling win rate within 5pp of viability prediction (96-97%). Reason: spring data alone doesn't capture summer regime. Toggle: `echo 'HOT_TAIL_HIGH_MODE=paper' >> .env.local && pm2 reload kardashev-web --update-env`.
- `LOW_TEMP_COLD_TAIL_MODE` — cold-tail (low-temp) signal-generation mode: `off` (default), `paper`, or `live`. LOW cold-tail = sell YES on bracket ranges ≥6°F BELOW forecast on low markets (the "deep cold" tail). Phase A viability 2026-05-04 returned strict NO-GO due to insufficient sample size (n=0 hits at -6°F+ in 61 events; low-temp signal-gen disable on 2026-04-10 stopped temp_bias accumulation). **Override: deployed in paper to gather forward data.** Mechanism (LOW-market cold-bias 65.57%) confirmed; real evaluation gate at +60-90 days with paper-resolved trades. Per-city blacklist starts empty; populate based on paper-mode forward data. Toggle: `echo 'LOW_TEMP_COLD_TAIL_MODE=paper' >> .env.local && pm2 reload kardashev-web --update-env`.
- `PRE_TRADE_ATM_GATE_MODE` — pre-trade atmospheric risk gate mode: `off` (default), `shadow`, or `active`. Runs `classifyPositionRisk` on every emitted tail-sell signal at signal-emission time with peak-hour atmospheric inputs (cloud cover, humidity, wind gust, pre-peak precip). `off`: no atmospheric inputs computed; classifier runs without atm and shadow-logs WARN/CRITICAL on drift/spread/boundary (preserves pre-2026-05-04 behavior). `shadow`: atmospheric inputs computed; classifier evaluates them; logs `[risk-shadow]` with `would-have-suppressed` suffix on paper quadrants but emits the signal unchanged. `active`: PAPER quadrants whose classifier returns WARN/CRITICAL are SUPPRESSED with `[risk-suppress]` log; **live cold-side HIGH (cold + high) is NEVER suppressed regardless of mode** — it always emits with shadow log only (mechanical tuple-based protection of the live earning path). On classifier exceptions the gate fails open and emits the signal unchanged. Trigger metadata persisted to `tail_sell_signals.atmosphericTriggers` for emitted signals (live shadow + paper-shadow). Rollout: deploy as `shadow` → +14 day validation comparing loss rates of triggered vs untriggered emitted signals → if triggered signals lose at >2x the rate of untriggered, flip to `active` for paper quadrants. Toggle: `echo 'PRE_TRADE_ATM_GATE_MODE=shadow' >> .env.local && pm2 reload kardashev-web --update-env`. See `memory/project-2026-05-04-state-and-experiments.md` (the 2026-05-04 locked plan file is archived — no longer in the repo).

### Environment Variables (Operational Alerts)
- `TELEGRAM_BOT_TOKEN` — bot token from @BotFather. Used by `src/lib/utils/telegram.ts` for operational alerts (currently: position risk monitor level transitions). NOT committed; set on droplet `.env.local` only. If missing, alerts silently fail-soft and console-log only — never throws.
- `TELEGRAM_CHAT_ID` — destination chat ID. Get via /start to your bot, then `https://api.telegram.org/bot<TOKEN>/getUpdates` to read your chat ID. Pair with `TELEGRAM_BOT_TOKEN`.
- `YES_SIGNALS_ENABLED` — controls the YES-side moratorium on probability-model inner/threshold-bracket signals in `src/lib/computeOpportunities.ts`. Default `false` (moratorium ON — historical YES win rate 13.8%, STRONG_YES 0/56). Set to `true` to lift for **data-capture purposes** to measure whether Phase 2 μ corrections shifted the YES asymmetry. **/weather-forecast filters YES non-HOLD recs from its public response**, so flipping ON does NOT surface YES recs to the trading UI; the audit trail lives at `/trading-readiness` under "Probability-Model Signals". `execute-tail-sells.ts` cron only handles tail-sell signals — flipping does NOT cause real trades. Toggle: `echo 'YES_SIGNALS_ENABLED=true' >> .env.local && pm2 reload kardashev-web --update-env`. Re-evaluate after 30+ post-lift YES trades resolve (decision rules in `docs/working-checklist.md`).
...
- `position_risk_snapshots` collection stores per-position risk classifications:
	- `{ signalId: 1, refreshedTimestamp: -1 }` for "latest snapshot per position" queries
	- `{ riskLevel: 1, refreshedTimestamp: -1 }` for "all CRITICAL" queries
	- `{ id: 1 }` unique
	- TTL index: `{ expiresAt: 1 }` with `expireAfterSeconds: 0` (14 days)
	- Written by `kardashev-position-monitor` PM2 cron (every 2h) — one row per pending tail-sell signal per cron run. Historical trail preserved for risk-timeline replay.
	- Read by `getOpenPositionRisks()` (consumed by `/api/weather/trading-readiness` for the Open Position Risk panel) and the same cron for transition detection (compare new classification vs prior to dedup Telegram alerts).
	- Schema in `src/lib/models/positionRiskTracker.ts` (PositionRiskSnapshot interface).
