...
- [x] Sweet Spot gate refresh spec (2026-04-26): written at `docs/work/sweet-spot-gates-refresh-spec.md`. New gate set: per-bucket (20-30¢ and 30-40¢ NO) cumulative-since-Phase-2 BSS + rolling-7d BSS, replacing the single 20-40¢ aggregate gate and dead `signalGeneration` gate. Sample threshold raised 50→30 (matches working-checklist viability criterion). Tri-state icons for sample-insufficient buckets. ~200 LOC build estimate, deferred to next week.
